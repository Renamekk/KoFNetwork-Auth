...
public interface LoginApprovalRepository {
     ...
     *
     * <p><b>Границей служит номер записи, а не её публичный идентификатор.</b>
     * Прежний вариант гасил «всё, кроме указанной», и две одновременные попытки
     * одного аккаунта — вход в игру и вход на сайте — гасили друг друга: каждая
     * успевала вставить свою строку, после чего каждая объявляла чужую устаревшей.
     * Обе кнопки уходили владельцу, обе были мертвы, и войти не удавалось ни
     * одним из способов до новой попытки. Условие {@code id < keepId} задаёт
     * порядок, одинаковый для обоих запросов: выживает та попытка, что записана
     * последней, — ровно то «новая отменяет предыдущую», ради которого метод и
     * существует.
     *
     * @param keepId номер записи, которую нужно сохранить; всё, что старше, гасится
